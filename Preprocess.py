...
class Preprocessor(Preprocessor_ML):
    ...
    def __init__(self, path, stratify=False):
        ...
        if stratify:
            X_train, X_test, y_train, y_test = \
                train_test_split(self.train_transaction, self.train_transaction['isFraud'],
                                 stratify=self.train_transaction['isFraud'])
    ...
    def GNN_Pre(self, EasyMode=True):
        ...
        Raw_data = self.R_GCN_Preprocess(OneHot=True, Fillna=True)
        Raw_data[['TransactionID', 'isFraud']].to_csv(os.path.join(self.output_dir, 'tags.csv'), index=False,
                                                      header=True)
    ...
    def R_GCN_Preprocess(self, OneHot=True, Fillna=True):
        ...
        if OneHot:
            OneHotFeature = pd.get_dummies(Raw_data[self.catagorical_column])
            OneHotFeature = pd.concat([OneHotFeature, Raw_data['TransactionID']], axis=1)
    ...
    def get_relations_and_edgelist(self, Raw_data, EasyMode):
        ...
        if EasyMode:
            ...
            edge_types = [x for x in edge_types if x[0] != 'i']
        # edge_types.remove('isFraud')
        edge_types.remove('TransactionDT')
        ...
        for etype in edge_types:
            ...
            edgelist.to_csv(os.path.join(self.output_dir, 'relation_{}_edgelist.csv').format(etype), index=False,
                            header=True)
